...
# Function to fetch and parse RSS feed from CNN
def fetch_cnn_news():
    feed_url = "http://rss.cnn.com/rss/edition.rss" # CNN RSS feed URL
...
# Container 1 - CNN News
with col1:
    st.sidebar.expander("CNN", expanded=True)
    cnn_news = fetch_cnn_news()
    for entry in cnn_news[:5]:  # Display the top 5 news articles
        st.sidebar.write(f"**[{entry.title}]({entry.link})**")
        st.sidebar.write(entry.summary)

# Container 2 - Yahoo Finance
with col2:
