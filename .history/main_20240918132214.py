...
# Function to get images from Yahoo Finance (if available)
def get_images_from_news(entries):
    images = []
    for entry in entries:
        # Use `entry.get('media_content')` or similar if available
        images.append(entry.get('media_thumbnail', {}).get('url', 'https://via.placeholder.com/150')) # Placeholder image
    return images
...
with col1:
    ...
    images = get_images_from_news(yahoo_news)
